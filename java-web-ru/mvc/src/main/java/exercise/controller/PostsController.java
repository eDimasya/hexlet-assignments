...
public class PostsController {
    ...
    public static void update(Context ctx) {
        ...
        try {
            post.setName(ctx.formParamAsClass("name", String.class)
                    .check(value -> value.length() >= 2, "Название не должно быть короче двух символов")
                    .get());
            post.setBody(ctx.formParamAsClass("body", String.class)
                    .check(value -> value.length() >= 10, "Пост должен быть не короче 10 символов")
                    .get());
            ...
            post.setName(ctx.formParam("name"));
            post.setBody(ctx.formParam("body"));
    ...
    public static void edit(Context ctx) {
        EditPostPage page = new EditPostPage(
                PostRepository.find(
                                ctx.pathParamAsClass("id", Long.class).get())
                        .orElseThrow(() ->
                                new NotFoundResponse("Post not found")));
